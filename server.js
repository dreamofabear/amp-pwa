...
// To generate content pages, run:
// cd content && curl -O http://localhost:4000/template/[1-30].amp.html
app.get('/template/:document', function(req, res) {
  fs.readFile(path.join(__dirname, 'content/template.mustache'), 'utf8', function (err, template) {
    if (err) {
      console.log(err);
    } else if (template) {
      var id = parseInt(req.params.document.split('.')[0]);

      var docWithId = getDocuments().filter(function(doc) {
        return doc.url == '/content/' + id + '.amp.html';
      });

      var values = docWithId[0];
      // Strip "/content" prefix for hero images.
      values.image = values.image.substring(9);
      res.send(mustache.render(template, values));
    }
  });
});
